...
export const login = async (credentials: LoginCredentials): Promise<ApiResponse<{
  ...
  const { password, verificationToken, resetPasswordToken, resetPasswordExpires, ...userWithoutSensitiveInfo } = user;
...
export const checkAuth = async (): Promise<ApiResponse<{
  ...
  const storedUser = localStorage.getItem('vive-medellin-user') || sessionStorage.getItem('vive-medellin-user');
